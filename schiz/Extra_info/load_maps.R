...
Df = readNIfTI(file.path(getwd(),'./Desktop/schiz/Extra_info/rs_fMRI_ica_maps.nii'))

# Load fMRI component numbers (as defined in reference [1] above)
fMRI_comp_ind = as.vector(read.csv(file="./Desktop/schiz/Extra_info/comp_ind_fMRI.csv",head=TRUE,sep=","))

# Select component to display:
#fMRI_ci = 6 # numbers 1..28
fMRI_ci = which(fMRI_comp_ind == 23) # numbers as defined in [1] above
...
# To change the current working directory use setwd()
Ds = readNIfTI(file.path(getwd(),'./Desktop/schiz/Extra_info/gm_sMRI_ica_maps.nii'))

# Load sMRI component numbers (as defined in reference [2] above)
sMRI_comp_ind = as.vector(read.csv(file="./Desktop/schiz/Extra_info/comp_ind_sMRI.csv",head=TRUE,sep=","))
